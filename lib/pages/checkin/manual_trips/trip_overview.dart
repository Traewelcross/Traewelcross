          ...
          if (snp.connectionState == .done) {
            ...
            return ListView.builder(
              ...
              itemBuilder: (ctx, idx) {
                ...
                return ListTile(
                  title: Text("${trip.lineName} (${trip.number}) - ${trip.operator?.name ?? "N/A"}"),
                  subtitle: RichText(
                    text: TextSpan(
                      children: [
                        TextSpan(text: "${trip.origin.name} (${trip.stopovers.first.departurePlanned})"),
                        const WidgetSpan(child: Icon(Icons.arrow_forward)),
                        TextSpan(text: "${trip.destination.name} (${trip.stopovers.last.arrivalPlanned})"),
                      ],
