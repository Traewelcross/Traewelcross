...
class TicketManager {
  static Future<Ticket?> getTicket(String uuid) async {
    ...
    final List<Ticket> ticket = (jsonDecode(tickets) as List<dynamic>)
  ...
  static Future<List<Ticket>?> getTickets({String? operator}) async {
    ...
    final String? tickets = await secureStorage.read(key: "tickets");
    if (tickets == null) return null;
    List<Ticket> ticket = (jsonDecode(tickets) as List<dynamic>)
        .map((t) => Ticket.fromJson(t as Map<String, dynamic>))
        .toList();
    if (operator == null) {
      return ticket;
  ...
  static Future<void> addTicket(Ticket ticket) async {
    ...
    if (tickets == null) {
      ...
      ticketL = (jsonDecode(tickets) as List<dynamic>)
          .map((t) => Ticket.fromJson(t as Map<String, dynamic>))
          .toList();
    }
    ticketL.add(ticket);
